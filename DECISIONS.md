...
- **Separated responsibilities**:
  - `BookerClient` = request mechanics (baseURL, headers, token)
...
- **Validation**:
  - Re-ran the API test after adding the guard; failures became deterministic and error output became actionable.
...
- **CI reality**: OS differences matter (fonts/rendering differ on macOS vs Linux).
...
- Snapshots are committed for Linux (CI); macOS snapshots may differ locally.
- **Validation**:
  - `npx playwright test tests/specs/saucedemo.visual.spec.ts --update-snapshots`
  - Verified by running in GitHub Actions and reviewing snapshot behavior.
...
- AI was used to accelerate scaffolding, but I:
  - fixed real runtime failures (safe JSON parsing)
  - addressed CI-specific issues (Linux baselines for visual snapshots)
  - migrated tooling to modern ESLint v9 config
  - verified behavior by running locally + validating in CI
  - documented trade-offs (OS-dependent snapshots and stability notes)

AI suggested starting points; I validated through execution, refactor, CI hardening, and documented decisions.